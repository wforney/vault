...
public class Base58CheckTests
{
    ...
    public void Encode()
    {
        foreach (Tuple<string, byte[]> tuple in this._testCases)
        {
            byte[] bytes = tuple.Item2;
            string expectedText = tuple.Item1;
            string actualText = Base58Check.EncodePlain(bytes);
    ...
    public void Decode()
    {
        foreach (Tuple<string, byte[]> tuple in this._testCases)
        {
            ...
            byte[] actualBytes = Base58Check.DecodePlain(text);
            Assert.Equal(BitConverter.ToString(expectedBytes), BitConverter.ToString(actualBytes));
        }
    }

    [Fact]
    public void DecodeInvalidChar() => Assert.Throws<FormatException>(() => Base58Check.DecodePlain("ab0"));

    [Fact]
    public void EncodeBitcoinAddress()
    {
        string actualText = Base58Check.Encode(AddressBytes);
