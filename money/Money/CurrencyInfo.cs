...
public partial class CurrencyInfo
{
    ...
    /// <returns></returns>
    public bool Equals(CurrencyInfo other) => other is not null && (ReferenceEquals(this, other) || Equals(other.Code, Code));
    ...
    public override bool Equals(object other) => other is not null && (ReferenceEquals(this, other) || (other.GetType() == typeof(CurrencyInfo) && Equals((CurrencyInfo)other)));
    ...
    public static bool operator ==(CurrencyInfo left, CurrencyInfo right) => Equals(left, right);
    ...
    public static implicit operator CurrencyInfo(Currency currency)
    {
        ...
        currencyInfo.NativeRegion = GetNativeRegionFromCurrencyCodeAndUserCulture(currency, out CultureInfo fallbackCulture);

        currencyInfo.DisplayCulture = GetDisplayCultureFromCurrencyCodeAndUserCulture(currency);

        if (fallbackCulture is not null && currencyInfo.DisplayCulture.IsNeutralCulture)
        {
    ...
    /// <summary>
    /// Performs an implicit conversion from <see cref="RegionInfo"/> to <see cref="CurrencyInfo"/>.
    /// </summary>
    ...
    /// <summary>
    /// Performs an implicit conversion from <see cref="CultureInfo"/> to <see cref="CurrencyInfo"/>.
    /// </summary>
    /// <param name="cultureInfo">The culture info.</param>
    /// <returns>The result of the conversion.</returns>
    public static implicit operator CurrencyInfo(CultureInfo cultureInfo) => new RegionInfo(cultureInfo.LCID);
    ...
    private static RegionInfo GetNativeRegionFromCurrencyCodeAndUserCulture(Enum currencyCode, out CultureInfo fallbackCulture)
    {
        ...
        if (locale is null)
        {
            ...
            // choose the most used equivalent for the native country as a fall-back
    ...
    private static CultureInfo GetDisplayCultureFromCurrencyCodeAndUserCulture(Enum currencyCode)
    {
        ...
        var nativeRegion = GetNativeRegionFromCurrencyCodeAndUserCulture(currencyCode, out _);
