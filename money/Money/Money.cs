...
{
    public override readonly int GetHashCode() => HashCode.Combine(this._createdDate, this._currencyInfo, this._override, this._places, this._units);
    ...
    public Money(CurrencyInfo currencyInfo, double value) : this(currencyInfo) => _units = ScaleUp(value);

    public Money(CurrencyInfo currencyInfo, decimal value) : this(currencyInfo) => _units = ScaleUp(value);

    public readonly DateTime CreatedDate => _createdDate;

    public readonly CurrencyInfo CurrencyInfo => _currencyInfo;

    public readonly int CompareTo(Money other) => other._units.CompareTo(_units);

    public bool Equals(Money other) => _createdDate.Equals(other._createdDate) && Equals(_currencyInfo, other._currencyInfo) && _override.Equals(other._override) && _places == other._places && _units == other._units;
    ...
    public readonly bool Equals(double other) => other == ScaleDownToDouble();

    public bool Equals(long other) => other == ScaleUp(ScaleDownToDouble());

    public override bool Equals(object other) => other is not null && other is Money money1 && Equals(money1);

    private readonly double ScaleDownToDouble()
    {
    ...
    private readonly decimal ScaleDownToDecimal()
    {
        var numberFormat = _currencyInfo.DisplayCulture.NumberFormat;
    ...
    /// </summary>
    /// <returns></returns>
    public readonly string DisplayNative() => ToString("c", _currencyInfo.DisplayCulture.NumberFormat);
    ...
    /// <returns></returns>
    public readonly string DisplayIn(CultureInfo displayCulture) => DisplayIn(displayCulture, true);
    ...
    public readonly string DisplayIn(CultureInfo displayCulture, bool disambiguateMatchingSymbol)
    {
    ...
    private static int CountDecimalPlaces(double input)
    {
        ...
        var places = value[(value.IndexOf('.') + 1)..].Length;
